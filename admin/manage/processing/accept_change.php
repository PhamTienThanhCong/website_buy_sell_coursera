...
where id_lesson='{$check['id_lesson']}' and id_course='{$check['id_course']}'";
    ...
    $sql = "delete from `update_course` WHERE id_lesson_update='$id_lesson_update'";
    $sql2 = "insert into `lesson` values('{$check['id_course']}','{$check['name_lesson']}',link = '{$check['link']}',type_link = '{$check['type_link']}',description_lesson = '{$check['description_lesson']}')";
    mysqli_query($connection, $sql);
    mysqli_query($connection, $sql2);
}
